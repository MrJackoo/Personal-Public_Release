...
function SentencePlayer(source, targetID)
    if QBCore.Functions.GetPlayer(targetID) ~= nil then
        TriggerClientEvent('qb-policejob:SentencePlayer', source, targetID)
    else
        TriggerClientEvent('QBCore:Notify', source, 'Player is not online..', 'error', 5000)
    end
end
...
RegisterNetEvent('qb-policejob:FinePlayer', function(targetID, amount)
    if QBCore.Functions.GetPlayer(targetID).Functions.RemoveMoney('bank', amount, "lsp-fine") then
        TriggerClientEvent('QBCore:Notify', targetID, 'You have been fined £'..amount..' by the Los Santos Police!', 'primary', 8500)
    else
        TriggerClientEvent('QBCore:Notify', targetID, 'Unknown Error..?', 'error', 5000)
    end
end)

-- Clothing system
RegisterServerEvent('police:server:takeclothing', function(playerId, type)
    local TargetedPlayer = QBCore.Functions.GetPlayer(playerId)
    TriggerClientEvent("police:client:takeoffclothing", TargetedPlayer.PlayerData.source, type, TargetedPlayer.PlayerData)
end)

-- QBCore.Commands.Add('sentence', 'Jail & Fine A Player (Police Only)', { {name = "ID", help = "Player ID"} }, true, function(source, args) 
--     local targetID = tonumber(args[1])
--     if targetID ~= nil then
--         SentencePlayer(source, targetID)
--     else
--         TriggerClientEvent('QBCore:Notify', source, 'Please specify a player id..', 'error', 5000)
--     end
-- end)
...
QBCore.Commands.Add("impound", "Impound car", {{name = "price", help = "Amount to charge for release"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("police:client:ImpoundCar", src, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)
...
QBCore.Commands.Add("seizecar", "Seize Car (Sends to police Impound)", {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("police:client:SeizeCar", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                OtherPlayer.Functions.AddMoney('bank', 500, "Police Tow Services")
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer.PlayerData.job.name == "lawyer" then
            OtherPlayer.Functions.AddMoney('bank', 500, "Police Lawyer Services")
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("success.tow_paid"), 'success')
...
QBCore.Commands.Add("panicbutton", "Emergency Button (Police, NHS Only)", {}, false, function(source, args)
    ...
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") and Player.PlayerData.job.onduty) then
        TriggerClientEvent("qb-policejob:client:ActivatePanicButton", source)
    else
        TriggerClientEvent('QBCore:Notify', source, 'You must be on duty to use that feature', 'error')
    end
end)
...
RegisterNetEvent('police:server:SentencePlayer', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("qb-policejob:SentencePlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)
...
QBCore.Functions.CreateUseableItem("zipties", function(source)
    ...
    TriggerClientEvent("police:client:ZiptiePlayer", src)
end)

QBCore.Functions.CreateUseableItem("weapon_knife", function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player.Functions.GetItemByName("weapon_knife") then return end
    TriggerClientEvent("police:client:CutZipties", src)
end)
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    ...
    Player.Functions.AddMoney("cash", tonumber(item.info.cash), "Moneybag Use")
end)
...
QBCore.Functions.CreateCallback('police:server:isPlayerDead', function(_, cb, playerId)
    ...
    if Player.PlayerData.metadata["inlaststand"] then
        cb(true)
    elseif Player.PlayerData.metadata["isdead"] then
        cb(true)
    else
        cb(false)
    end
...
RegisterNetEvent('police:server:TakeOutImpound', function(plate, garage)
    ...
    local targetCoords = Config.PoliceimpoundLocation
...
RegisterNetEvent('police:server:ZiptiePlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    ...
    Player.Functions.RemoveItem('zipties', 1)
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['zipties'], 'remove')
    TriggerClientEvent("police:client:GetZiptied", CuffedPlayer.PlayerData.source, Player.PlayerData.source)
end)
...
RegisterNetEvent('police:server:UnCuffPlayer', function(playerId)
    ...
    TriggerClientEvent("police:client:GetUnCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source)
end)

RegisterNetEvent('police:server:CutZipties', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local CuffedPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("weapon_knife")) then return end

    TriggerClientEvent("police:client:Releasezipties", CuffedPlayer.PlayerData.source, Player.PlayerData.source)
end)

RegisterNetEvent('police:server:SoftCuffPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    TriggerClientEvent("police:client:SoftCuff", CuffedPlayer.PlayerData.source, Player.PlayerData.source)
end)
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isziptied"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
RegisterNetEvent('police:server:KidnapPlayer', function(playerId)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isziptied"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
...
--     TriggerEvent('qb-banking:society:server:DepositMoney', Config.BillCost, 'police')
--     TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("info.fine_received", {fine = price}))
-- end)

RegisterNetEvent('police:server:FinePlayer', function(playerId, price)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if #(playerCoords - targetCoords) > 10 then
        return TriggerClientEvent('QBCore:Notify', src, 'The player must be within 10 Meters', 'error')
    end
    ...
    OtherPlayer.Functions.RemoveMoney("bank", price, "Fine Payment")
    exports['qb-management']:AddMoney("police", price)
    
    TriggerClientEvent('QBCore:Notify', src, 'You have fined: '..playerId..' £'..price, 'success')
    TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, "You have been fined: £"..price)
end)
...
RegisterNetEvent('police:server:SetZiptiedStatus', function(isZiptied)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
        Player.Functions.SetMetaData("isziptied", isZiptied)
    end
end)
...
RegisterNetEvent('police:server:RobPlayer', function(playerId)
    ...
    Player.Functions.AddMoney("cash", money, "Stole Money (Robbery)")
    SearchedPlayer.Functions.RemoveMoney("cash", money, "Lost Money (Robbery)")
...
RegisterNetEvent('police:server:SeizeVehicle', function(plate, body, engine, fuel)
    local src = source
    if IsVehicleOwned(plate) then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Vehicle has Been Seized", "success", 5000)
    end
end)

RegisterNetEvent('police:server:ImpoundCar', function(plate, price, body, engine, fuel)
    local src = source
    ...
    if IsVehicleOwned(plate) then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Vehicle has been sent to the Impound", "primary", 5000)
    end
end)
...
RegisterNetEvent('evidence:server:AddFingerprintToInventory', function(fingerId, fingerInfo)
    ...
    local evidencebag = Player.Functions.GetItemByName("empty_evidence_bag")
    
    if evidencebag ~= nil and evidencebag.amount > 1 then
        Player.Functions.AddItem("filled_evidence_bag", 1, false, fingerInfo)
        TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items["filled_evidence_bag"], "add")
...
AddEventHandler('wk:onPlateLocked', function( cam, plate, index )
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if Plates and Plates[plate:upper()] then
            if Plates[plate:upper()].isflagged then
                TriggerClientEvent('QBCore:Notify', src, 'You have scanned Plate: ['..plate:upper()..'] and it has came back as flagged for: '..Plates[plate:upper()].reason, 'success', 10000)
                
            else
                TriggerClientEvent('QBCore:Notify', src, "Vehicle Not Flagged", 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Vehicle Not on record", 'error')
        end
    end 
end)
...
QBCore.Commands.Add("registermdw", "RegisterMDW", {}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent('axiomPD:client:mdwAccess', src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)
...
RegisterNetEvent('axiomPD:server:mdwAccess', function(citizenid, fname, lname, callsign, phone, rank)
    
    local department = 'Mission Row'  
    local alias = 'Officer'
    local image = 'https://i.imgur.com/RIDbvqV.png'

    MySQL.Async.insert('INSERT INTO `drx_mdw_officers` (`citizenid`, `firstname`, `lastname`, `callsign`, `alias`, `phone`, `department`, `rank`, `image`, `roles`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
    {
        citizenid,
        fname,
        lname,
        ...
        rank,
        image,
        '[{"access":true,"id":1,"role":"Edit Profiles"},{"access":true,"id":2,"role":"Edit Incidents"},{"access":true,"id":3,"role":"Create Incidents"},{"access":true,"id":8,"role":"Edit Evidence"},{"access":true,"id":9,"role":"Create Evidence"},{"access":true,"id":11,"role":"Edit Vehicles"}]'
    })
end)

QBCore.Functions.CreateUseableItem("breaching_charge", function(source, _)
    local Player = QBCore.Functions.GetPlayer(source)
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty and Player.PlayerData.job.grade.level >= 7) then
        TriggerClientEvent("qb-police:client:breachdoor", source)
    else
        TriggerClientEvent('QBCore:Notify', source, "You need to be a Chief Superintendent or above", "error")
    end
end)