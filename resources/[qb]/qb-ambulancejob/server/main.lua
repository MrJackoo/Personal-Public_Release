...
RegisterNetEvent('hospital:server:LayinBed', function(bedId, isRevive)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	TriggerClientEvent('hospital:client:SendToBed', src, bedId, Config.Locations["beds"][bedId], isRevive)
	TriggerClientEvent('hospital:client:SetBed', -1, bedId, true)
...
RegisterServerEvent('qb-ambulancejob:server:ShadyMedical', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	for k, v in pairs(Config.Locations["shadybed"]) do
		TriggerClientEvent('qb-ambulancejob:ShadyMedicalBed', src, k, v, true)
		return
	end
end)

RegisterServerEvent('qb-ambulancejob:server:ShadyMedicalBill', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player ~= nil then
		Player.Functions.RemoveMoney("bank", 7500, "shady-medical")
		TriggerClientEvent('QBCore:Notify', src, 'You have been charged £2500 for using these medical facilities', 'success', 5000)
	end
end)
...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	...
	if Patient then
		if Player.PlayerData.job.name =="ambulance" then
			...
			TriggerEvent("qb-log:server:CreateLog", "medical", "Healed Player", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ ".. GetPlayerName(src) .. " - Citizen ID: "..Player.PlayerData.citizenid.." | ID:"..src.." ] healed **"..Patient.PlayerData.charinfo.firstname .. " ".. Patient.PlayerData.charinfo.lastname.. "** [ ".. GetPlayerName(playerId) .. " - Citizen ID: "..Patient.PlayerData.citizenid.." | ID:"..playerId.." ]")
		end
	end
end)

RegisterNetEvent('hospital:server:GivingMorphine', function(playerId)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local Patient = QBCore.Functions.GetPlayer(playerId)
	if Patient then
		if Player.PlayerData.job.name =="ambulance" then
			TriggerClientEvent("hospital:client:OnMorphine", Patient.PlayerData.source, "full")
			-- TriggerEvent("qb-log:server:CreateLog", "medical", "Gave Player Morphine", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ ".. GetPlayerName(src) .. " - Citizen ID: "..Player.PlayerData.citizenid.." | ID:"..src.." ] healed **"..Patient.PlayerData.charinfo.firstname .. " ".. Patient.PlayerData.charinfo.lastname.. "** [ ".. GetPlayerName(playerId) .. " - Citizen ID: "..Patient.PlayerData.citizenid.." | ID:"..playerId.." ]")
		end
	end
end)
...
RegisterNetEvent('hospital:server:SetDoctor', function()
    ...
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
			print("Adding another doctor to amount")
            amount += 1
			print("new amount is now:", amount)
        end
	end
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId)
	...
	local Patient = QBCore.Functions.GetPlayer(playerId)
	if Patient then
		Player.Functions.AddMoney('bank', 250, 'Medical Services (Revive Reward)')
		TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
		TriggerEvent("qb-log:server:CreateLog", "medical", "Revived Player", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ ".. GetPlayerName(src) .. " - Citizen ID: "..Player.PlayerData.citizenid.." | ID:"..src.." ] revived **"..Patient.PlayerData.charinfo.firstname .. " ".. Patient.PlayerData.charinfo.lastname.. "** [ ".. GetPlayerName(playerId) .. " - Citizen ID: "..Patient.PlayerData.citizenid.." | ID:"..playerId.." ]")
	end
end)
...
RegisterNetEvent('hospital:server:removeFirstaid', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
...
RegisterNetEvent('hospital:server:removeOxy', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
