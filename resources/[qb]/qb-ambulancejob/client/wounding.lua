...
RegisterNetEvent("hospital:client:UseiFak", function()
    ...
    QBCore.Functions.Progressbar("use_ifak", "Applying iFak", 5000, false, true, {
...
RegisterNetEvent('hospital:client:UseOxy', function()
    ...
    if armour < 100 then
    QBCore.Functions.Progressbar("use_oxy", "Taking Oxy...", 2000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerServerEvent("hospital:server:removeOxy")
        ...
        QBCore.Functions.Notify("You already have maximum armor", "error", 3000)
    end
end)
...
RegisterNetEvent('hospital:client:UseFirstAid', function()
    ...
    local health = GetEntityHealth(ped)
    if health < 200 then
    QBCore.Functions.Progressbar("use_firstaid", "Using First Aid..", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        QBCore.Functions.Notify("You don\'t need to heal", "error", 3000)
    end
end)
