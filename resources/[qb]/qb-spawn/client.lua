...
RegisterNUICallback('setCam', function(data, cb)
    ...
    elseif type == "jail" then
        SetCam(QB.SpawnsJailed['prison'].coords)
    elseif type == "police" then
        SetCam(QB.PoliceSpawns['mrpd'].coords)
    elseif type == "ambulance" then
        SetCam(QB.NHSSpawns['pillbox'].coords)
    end
    cb('ok')
end)
...
RegisterNUICallback('spawnplayer', function(data, cb)
   ...
   -- if QBCore.Functions.GetPlayerData().metadata["injail"] == 0 then
    ...
    elseif type == "normal" then
       ...
       -- end
    elseif type == "police" then
        local pos = QB.PoliceSpawns['mrpd'].coords
        PreSpawnPlayer()
        SetEntityCoords(ped, pos.x, pos.y, pos.z)
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
    ...
    elseif type == "hospital" then
        local pos = QB.NHSSpawns['pillbox'].coords
        PreSpawnPlayer()
        SetEntityCoords(ped, pos.x, pos.y, pos.z)
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
       ...
       -- end
    elseif type == "jail" then
        local pos = QB.SpawnsJailed['prison'].coords
        PreSpawnPlayer()
        SetEntityCoords(ped, pos.x, pos.y, pos.z)
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
    ...
    Citizen.Wait(500)
    TriggerEvent('QBCore:Client:DisableDuringNearestPostalCharacterSelection', postalhidden) --- Used to Disable Nearest Postal on Char select
    cb('ok')
end)
