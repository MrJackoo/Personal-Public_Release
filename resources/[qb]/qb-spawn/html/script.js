$(document).ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (data.action == "setuppolice") {
            setupPolice(data.locations, data.houses)
        }

        if (data.action == "setupnhs") {
            setupNHS(data.locations, data.houses)
        }

        if (data.action == "setupJail") {
            setupJail(data.locations)
        }
...
// Faction Spawns
function setupPolice(Stations, myHouses) {
    var parent = $('.spawn-locations')
    $(parent).html("");
    ...
    setTimeout(function(){
        ...
        $.each(Stations, function(index, location){
            $(parent).append('<div class="location" id="location" data-location="'+location.location+'" data-type="police" data-label="'+location.label+'"><p><span id="'+location.location+'">'+location.label+'</span></p></div>')
        });
...
function setupNHS(hospitals, myHouses) {
    var parent = $('.spawn-locations')
    $(parent).html("");
    ...
    setTimeout(function(){
        ...
        $.each(hospitals, function(index, location){
            $(parent).append('<div class="location" id="location" data-location="'+location.location+'" data-type="hospital" data-label="'+location.label+'"><p><span id="'+location.location+'">'+location.label+'</span></p></div>')
        });
...
function setupJail(jail) {
    var parent = $('.spawn-locations')
    $(parent).html("");

    $(parent).append('<div class="loclabel" id="location" data-jail="null" data-type="lab" data-label="Choose a Prison"><p><span id="null">You are in prison</span></p></div>')

    $.each(jail, function(index, jail){
        $(parent).append('<div class="location" id="location" data-jail="'+jail.name+'" data-type="jail" data-label="'+jail.label+'"><p><span id="'+jail.name+'">'+jail.label+'</span></p></div>')
    });
