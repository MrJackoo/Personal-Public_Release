...
RegisterServerEvent('qb-crypto:server:ExchangeSuccess', function(LuckChance)
    ...
    if ItemData ~= nil then
        ...
        local Amount = (math.random(1611111, 2999999) / DeelNumber)
        if LuckChance >= 75 then 
            Amount = (math.random(2999999, 4999999) / DeelNumber) 
        end

        Player.Functions.RemoveItem("cryptostick", 1)
        Player.Functions.AddMoney('crypto', Amount, 'Crypto Exchnage')
...
QBCore.Functions.CreateCallback('qb-crypto:server:BuyCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.bank >= total_price then
        ...
        Player.Functions.RemoveMoney('bank', total_price, 'Crypto: Crypto Purchased')
        ...
        Player.Functions.AddMoney('crypto', tonumber(data.Coins), 'Crypto Purchase')
        cb(CryptoData)
    else
        cb(false)
    end
...
QBCore.Functions.CreateCallback('qb-crypto:server:SellCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        Player.Functions.RemoveMoney('crypto', tonumber(data.Coins), 'Crypto: Crypto Sold')
        ...
        Player.Functions.AddMoney('bank', amount, 'Crypto Sold')
        cb(CryptoData)
    else
        cb(false)
    end
...
QBCore.Functions.CreateCallback('qb-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            Player.Functions.RemoveMoney('crypto', tonumber(data.Coins), 'Crypto: Transferred Crypto')
            ...
            if Target ~= nil then
                Target.Functions.AddMoney('crypto', tonumber(data.Coins), 'Crypto Transfer')
