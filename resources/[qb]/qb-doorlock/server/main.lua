...
RegisterNetEvent('qb-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	-- print("doorID: ",doorID," locked: ",locked," src: ",src," used lockpick: ",usedLockpick," unlock anyway: ",unlockAnyway," enable sound: ",enableSounds," enable anim: ",enableAnimation," sentsource: ",sentSource)
...
RegisterNetEvent('qb-police:server:explode', function(closestDoor)

	local Player = QBCore.Functions.GetPlayer(source) 
    if Player.Functions.RemoveItem('breaching_charge', 1) then
    	TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['breaching_charge'], "remove")
		TriggerClientEvent("qb-police:client:explosion", source, closestDoor)
		TriggerClientEvent("qb-police:client:explosionNotice", source, closestDoor)
	else
		TriggerClientEvent('QBCore:Notify', src, 'Error: No Charge Found!', 'error', 5000)
	end
end)
