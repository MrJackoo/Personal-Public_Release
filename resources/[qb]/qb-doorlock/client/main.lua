...
local alreadybreachingdoor = false

RegisterNetEvent('qb-police:client:breachdoor', function()
	if not closestDoor.data or not next(closestDoor.data) or alreadybreachingdoor or not closestDoor.data.locked or PlayerData.metadata['isdead'] or PlayerData.metadata['ishandcuffed'] or PlayerData.metadata['isziptied'] then
		return QBCore.Functions.Notify('Error: Door already unlocked or already being breached!', 'error', 5000)
	end

	alreadybreachingdoor = true

	TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base base", "idle_a", 1.0, 1.0, 5.0, 15, 1.0, true, true, true)
	
	QBCore.Functions.Progressbar("plantbreachingcharge", "Planting Breaching Charge", 1500, false, true, {
	...
	function() -- Done
		...
		-- exports['ps-ui']:VarHack(function(success)
			-- if success then
			-- 	print("success")
			-- 	TriggerServerEvent('qb-police:server:explode', closestDoor)
			-- else
			-- 	print("fail")
			-- 	alreadybreachingdoor = false
			-- end
		-- end, 2, 5) -- Number of Blocks, Time (seconds)

		local success = exports['qb-lock']:StartLockPickCircle(4,10)
		if success then
			TriggerServerEvent('qb-police:server:explode', closestDoor)
		else
			alreadybreachingdoor = false
			QBCore.Functions.Notify("Failed to plant Charge", "error")
		end
...
RegisterNetEvent('qb-police:client:explosion', function(door)

	local explosivemodel = 'prop_bomb_01'
	RequestModel(explosivemodel)
	while not HasModelLoaded(explosivemodel) do
	    Wait(0)
	end
	
	QBCore.Functions.Notify('Charge Planted: Stand Clear for detonation', 'primary', 5000)

	if door.data.BreachCoords then
		local charge = CreateObject(explosivemodel, door.data.BreachCoords, false, false, false)
		Citizen.Wait(9650)
		AddExplosion(door.data.BreachCoords, "EXPLOSION_STICKYBOMB", false, true, false, true)
		TriggerServerEvent('qb-doorlock:server:updateState', door.id, false, false, false, true)
		DeleteEntity(charge)
		DeleteObject(charge)
		alreadybreachingdoor = false
	elseif door.data.doors then
		local charge = CreateObject(explosivemodel, door.data.doors[1].objCoords.x, door.data.doors[1].objCoords.y, door.data.doors[1].objCoords.z, false, false, false)
		Citizen.Wait(9650)
		AddExplosion( door.data.doors[1].objCoords.x,  door.data.doors[1].objCoords.y, door.data.doors[1].objCoords.z, "EXPLOSION_STICKYBOMB", false, true, false, true)
		TriggerServerEvent('qb-doorlock:server:updateState', door.id, false, false, false, true)
		DeleteEntity(charge)
		DeleteObject(charge)
		alreadybreachingdoor = false
	elseif door.data.objCoords then
		local charge = CreateObject(explosivemodel, door.data.objCoords.x, door.data.objCoords.y, door.data.objCoords.z, false, false, false)
		Citizen.Wait(9650)
		AddExplosion( door.data.objCoords.x,  door.data.objCoords.y, door.data.objCoords.z, "EXPLOSION_STICKYBOMB", false, true, false, true)
		TriggerServerEvent('qb-doorlock:server:updateState', door.id, false, false, false, true)
		DeleteEntity(charge)
		DeleteObject(charge)
		alreadybreachingdoor = false
...
RegisterNetEvent('qb-police:client:explosionNotice', function(door)
	local count = 10
	for i=1, 9 do
		count = count - 1 
		QBCore.Functions.Notify('Charge Planted: Stand Clear for detonation in: '..count..' seconds', 'primary', 2000)
		Citizen.Wait(1000)	
	end
end)
...
local function lockpickFinish(success)
	if success then
		...
		Wait(1800)
		TriggerServerEvent('qb-doorlock:server:updateState', closestDoor.id, true, false, false, true) -- Broadcast new state of the door to everyone
	else
		QBCore.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
		if math.random(1,100) <= 17 then
			if usingAdvanced then
				TriggerServerEvent("consumables:server:removeAdvanced")
				TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["advancedlockpick"], "remove")
			else
				TriggerServerEvent("consumables:server:removeLockpick")
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
	if not closestDoor.data or not next(closestDoor.data) or PlayerData.metadata['isdead'] or PlayerData.metadata['ishandcuffed'] or PlayerData.metadata['isziptied'] or (not closestDoor.data.pickable and not closestDoor.data.lockpick) or not closestDoor.data.locked then return end
...
RegisterCommand('toggledoorlock', function()
	...
	local busyCheck = PlayerData.metadata['isdead'] or PlayerData.metadata['inlaststand'] or PlayerData.metadata['ishandcuffed'] or PlayerData.metadata['isziptied']
...
RegisterKeyMapping('remotetriggerdoor', Lang:t("general.keymapping_remotetriggerdoor"), 'keyboard', 'E')
