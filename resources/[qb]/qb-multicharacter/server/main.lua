    ...
    MySQL.query('SELECT * FROM players WHERE citizenid = ?', {citizenid}, function(result)
        if result[1] then
            for i = 1, (#result), 1 do
                ...
                if result[i].metadata['injail'] == 0 then --- Deletes a character if the player has no jail time.
                    QBCore.Player.DeleteCharacter(src, citizenid)
                else
                    TriggerClientEvent('QBCore:Notify', src, 'You cannot delete a jailed citizen, this citizen has '..tostring(result[i].metadata['injail'])..' months remaining.', 'error')
                end
                break
            end
        end
