...
local function Setupwhitelistmenu()
    if (QBCore.Functions.GetPlayerData().job.isboss and (QBCore.Functions.GetPlayerData().job.name == "police" or QBCore.Functions.GetPlayerData().job.name == "ambulance")) then
        whitelistid = AddOption({
            id = 'togglewhitelist',
            title = 'Whitelist',
            icon = 'tablet',
            type = 'client',
        ...
        if whitelistid ~= nil then
            RemoveOption(whitelistid)
            whitelistid = nil
        end
    end
end
...
RegisterCommand('radialmenu', function()
    if ((IsDowned() and IsPoliceOrEMS()) or not IsDowned()) and not PlayerData.metadata["ishandcuffed"] and not PlayerData.metadata["isziptied"] and not IsPauseMenuActive() and not inRadialMenu then
