...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
    ...
    if #(playerCoords - Config.Registers[register].coords) > 3.0 or (not Config.Registers[register].robbed and not isDone) or (Config.Registers[register].time <= 0 and not isDone) or (not Config.Registers[register].failed and not isDone) then
        -- return DropPlayer(src, "Attempted exploit abuse")
        return QBCore.Functions.Notify(src, "You are too far away", "error")
    end
    ...
    if isDone then
        ...
        Player.Functions.AddMoney("cash", amount, 'Store Robbery: Stole from Register')
        TriggerClientEvent("qb-storerobbery:client:pedattack", src, true)
...
RegisterNetEvent('qb-storerobbery:server:setRegisterStatus', function(register, failed)
    if failed then
        Config.Registers[register].failed = true
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    local playerPed = GetPlayerPed(src)
    local playerCoords = GetEntityCoords(playerPed)
    if #(playerCoords - Config.Safes[safe].coords) > 3.0 or Config.Safes[safe].robbed then
        return QBCore.Functions.Notify(src, "You are too far away", "error")
    end

    Player.Functions.AddMoney("cash", Config.SafeCash, 'Store Robbery: Stole from safe')
...
RegisterNetEvent('qb-storerobbery:server:unlucky', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-storerobbery:server:notrobnow', function()
    local src = source
    TriggerClientEvent("qb-storerobbery:client:pedattack", src)
end)
    ...
    while true do
        ...
        for k in ipairs(Config.Registers) do
            ...
            if Config.Registers[k].time > 0 and (Config.Registers[k].time - Config.tickInterval) >= 0 then
                ...
                if Config.Registers[k].failed then
                    Config.Registers[k].time = 0
                    Config.Registers[k].failed = false
                    toSend[#toSend+1] = Config.Registers[k]
                end
