...
local AlertCops = function()
	...
	if alertPolice >= 75 then
		local data = exports['cd_dispatch']:GetPlayerInfo()
		TriggerServerEvent('cd_dispatch:AddNotification', {
			job_table = {'police'}, 
			coords = data.coords,
			title = 'Oxy Dealing',
			message = 'Possible oxy drug deal taking place on '..data.street,
			flash = 0,
...
local CreateDropOff = function()
    ...
    --     sender = 'UNKNOWN',
    --     subject = 'DELIVERY',
    --     message = 'Make your way to the drop-off check your map(💊) and keep an eye out for the feds',
    -- })
	...
	dropOffArea:onPlayerInOut(function(isPointInside, point)
		if isPointInside then
			if oxyPed == nil then
				...
				-- 	sender = 'UNKNOWN',
				-- 	subject = 'HURRY UP',
				-- 	message = 'Make the Delivery man hurry up',
				-- })
...
RegisterNetEvent('qb-oxyruns:client:DeliverOxy', function()
	...
	if #(GetEntityCoords(ped) - GetEntityCoords(oxyPed)) < 5.0 then
		...
		if dropOffCount == Config.RunAmount then
			...
			-- 	sender = 'UNKNOWN',
			-- 	subject = 'NICE JOB',
			-- 	message = 'You are done delivering, go back to the pharmacy..',
			-- })
			...
			-- 	sender = 'UNKNOWN',
			-- 	subject = 'NEW JOB',
			-- 	message = 'Delivery was good, you will be updated with the next drop-off..',
			-- })
