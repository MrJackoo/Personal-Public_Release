...
exports('qb-management', AddMoney)
...
exports('qb-management', RemoveMoney)
...
RegisterNetEvent("qb-bossmenu:server:withdrawMoney", function(amount)
	...
	if RemoveMoney(job, amount) then
		...
		if amount >= 25000 then
			TriggerEvent("qb-log:server:CreateLog", "bossmenu", "Funds Withdrawal ("..job..")", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ "..GetPlayerName(src) .. "(** Citizen ID: "..Player.PlayerData.citizenid.." | ID: "..Player.PlayerData.source..")** withdrew **£" .. amount .. " from the " .. job .. " account", true)
		else
			TriggerEvent("qb-log:server:CreateLog", "bossmenu", "Funds Withdrawal ("..job..")", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ "..GetPlayerName(src) .. "(** Citizen ID: "..Player.PlayerData.citizenid.." | ID: "..Player.PlayerData.source..")** withdrew **£" .. amount .. " from the " .. job .. " account", false)
		end
...
RegisterNetEvent("qb-bossmenu:server:depositMoney", function(amount)
	...
	if Player.Functions.RemoveMoney("cash", amount, 'BossMenu: Deposit') then
		local job = Player.PlayerData.job.name
		AddMoney(job, amount)
		if amount >= 25000 then
			TriggerEvent("qb-log:server:CreateLog", "bossmenu", "Funds Deposit ("..job..")", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ "..GetPlayerName(src) .. "(** Citizen ID: "..Player.PlayerData.citizenid.." | ID: "..Player.PlayerData.source..")** deposited **£" .. amount .. " into the " .. job .. " account", true)
		else
			TriggerEvent("qb-log:server:CreateLog", "bossmenu", "Funds Deposit ("..job..")", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ "..GetPlayerName(src) .. "(** Citizen ID: "..Player.PlayerData.citizenid.." | ID: "..Player.PlayerData.source..")** deposited **£" .. amount .. " into the " .. job .. " account", false)
		end
	...
	if players[1] ~= nil then
		for _, value in pairs(players) do
			...
			if isOnline then
				...
				whitelistingflags = isOnline.PlayerData.metadata['jobflags']
				}
			else
				employees[#employees+1] = {
				...
				whitelistingflags = json.decode(value.metadata).jobflags
				}
			end
		end
...
RegisterNetEvent('qb-bossmenu:server:GradeUpdate', function(data)
	...
	if Employee then
		if Employee.Functions.SetJob(Player.PlayerData.job.name, data.grade) then
			...
			TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "Your Rank has changed to: " ..data.gradename..".", "success")
		else
...
-- flag change @jackbatchiee
RegisterNetEvent('qb-bossmenu:server:FlagUpdate', function(data)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local Employee = QBCore.Functions.GetPlayerByCitizenId(data.cid)
	...
	if Employee then
		if Employee.Functions.ChangeFlagLevel(data.flagname, data.grade) then
			TriggerClientEvent('QBCore:Notify', src, "Sucessfully changed flag whitelisting  ", "success")
			TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "Your whitelisting flag for " ..data.flagname.. " has changed to: " ..string.upper(data.newlevel)..".", "success")
		else
			TriggerClientEvent('QBCore:Notify', src, "Whitelisting flag does not exist.", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Civilian not in city.", "error")
	end
