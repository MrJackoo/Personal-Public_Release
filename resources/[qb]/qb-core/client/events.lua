...
RegisterNetEvent('QBCore:Command:DeleteVehicle', function(range)
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsUsing(ped)
    if veh ~= 0 then
        NetworkRequestControlOfNetworkId(veh)
        SetEntityAsMissionEntity(veh, true, true)
        TriggerEvent('persistent-vehicles/delete-vehicle', veh)
        DeleteVehicle(veh)
        DeleteEntity(veh)
    else
        ...
        for _, v in pairs(vehicles) do
            if #(pcoords - GetEntityCoords(v)) <= range then
                NetworkRequestControlOfNetworkId(v)
                SetEntityAsMissionEntity(v, true, true)
                TriggerEvent('persistent-vehicles/delete-vehicle', v)
            end
        end
    end
end)
