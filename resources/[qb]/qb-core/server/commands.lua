...
QBCore.Commands.Add('addpermission', 'Give Player Permissions (Staff lead Only)', { { name = 'id', help = 'ID of player' }, { name = 'permission', help = 'Permission level' } }, true, function(source, args)
...
QBCore.Commands.Add('dv', 'Delete Vehicle (Admin Only)', { { name = 'range', help = 'Range to delete vehicles' } }, true, function(source, args)
    if args[1] == nil then args[1] = 3 end
    local range = tonumber(args[1])
    if range > 100 then 
        TriggerClientEvent('QBCore:Notify', source, "Range must be below 100", 'error')
    else
        TriggerClientEvent('QBCore:Command:DeleteVehicle', source, range)
    end
end, 'admininistrator')
...
QBCore.Commands.Add('givemoney', 'Give A Player Money (Admin Only)', { { name = 'id', help = 'Player ID' }, { name = 'moneytype', help = 'Type of money (cash, bank, crypto)' }, { name = 'amount', help = 'Amount of money' } }, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
        Player.Functions.AddMoney(tostring(args[2]), tonumber(args[3]), 'ADMIN: GIVE MONEY CMD')
    else
        TriggerClientEvent('QBCore:Notify', source, Lang:t('error.not_online'), 'error')
    end
end, 'admininistrator')
...
QBCore.Commands.Add('me', 'Show local message', {{name = 'message', help = 'Message to respond with'}}, false, function(source, args)
    ...
    for i=1, #Players do
        ...
        if target == ped or #(pCoords - tCoords) < 20 then
            ...
            TriggerEvent("qb-log:server:CreateLog", "me", "Me", "white", "**"..GetPlayerName(source).."** Message: " ..msg, false)
        end
    end
end, 'user')
