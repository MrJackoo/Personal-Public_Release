...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    -- This populates the playerdata metadata with job whitelisting flags @jackbatchiee
    PlayerData.metadata['jobflags'] = PlayerData.metadata['jobflags'] or {
        ['firearms'] = 0,
        ['traffic'] = 0,
        ['npas'] = 0,
        ['dsu'] = 0,
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    -- This block of code ensures that the players jobflags change @jackbatchiee
    function self.Functions.ChangeFlagLevel(flagtype,number)
        if not number then return end
        number = tonumber(number)
        flagtochange = tostring(flagtype)
        self.PlayerData.metadata['jobflags'][flagtochange] = number
        -- print('I think jack has done it correctly ' .. number .. ' with the flag changed ' .. flagtochange)
        if not self.Offline then
            TriggerEvent('QBCore:Server:OnFlagUpdate', self.PlayerData.source, self.PlayerData.metadata['jobflags'])
            TriggerClientEvent('QBCore:Client:OnFlagUpdate', self.PlayerData.source, self.PlayerData.metadata['jobflags'])
            self.Functions.UpdatePlayerData()
        end
        return true
    ...
    function self.Functions.AddMoney(moneytype, amount, reason)
        reason = reason or 'unknown/Spawned In'
        moneytype = moneytype:lower()
        amount = tonumber(amount)
        if amount < 0 then return end
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            if amount > 100000 then
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Added Money', 'green', '**' .. GetPlayerName(self.PlayerData.source) .. ' (Citizen ID: ' .. self.PlayerData.citizenid .. ' | ID: ' .. self.PlayerData.source .. ')** £' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason, true)
            else
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Added Money', 'green', '**' .. GetPlayerName(self.PlayerData.source) .. ' (Citizen ID: ' .. self.PlayerData.citizenid .. ' | ID: ' .. self.PlayerData.source .. ')** £' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
            end
            TriggerClientEvent('hud:client:OnMoneyChange', self.PlayerData.source, moneytype, amount, false)
        end
  ...
  -- NPWD Testing
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            if amount > 100000 then
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Remove Money', 'red', '**' .. GetPlayerName(self.PlayerData.source) .. ' (Citizen ID: ' .. self.PlayerData.citizenid .. ' | ID: ' .. self.PlayerData.source .. ')** £' .. amount .. ' (' .. moneytype .. ') removed | New ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason, true)
            else
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Remove Money', 'red', '**' .. GetPlayerName(self.PlayerData.source) .. ' (Citizen ID: ' .. self.PlayerData.citizenid .. ' | ID: ' .. self.PlayerData.source .. ')** £' .. amount .. ' (' .. moneytype .. ') removed | New ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
            end
            TriggerClientEvent('hud:client:OnMoneyChange', self.PlayerData.source, moneytype, amount, true)
            if moneytype == 'bank' then
                TriggerClientEvent('qb-phone:client:RemoveBankMoney', self.PlayerData.source, amount)
    ...
    --         TriggerEvent('qb-log:server:CreateLog', 'money', 'Set Player Money', 'green', '**'..GetPlayerName(self.PlayerData.source) .. ' (Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..')** Set £'..amount .. ' ('..moneytype..') new '..moneytype..' balance: '..self.PlayerData.money[moneytype], true)
    --     end
    ...
    function self.Functions.SetMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            TriggerEvent('qb-log:server:CreateLog', 'money', 'Set Player Money', 'green', '**'..GetPlayerName(self.PlayerData.source) .. ' (Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..')** Set £'..format_int(amount) .. ' ('..moneytype..') new '..moneytype..' balance: '..self.PlayerData.money[moneytype], true)
    end
    ...
    self.Functions.AddItem = function(item, amount, slot, info)
        ...
        if (totalWeight + (itemInfo['weight'] * amount)) <= QBCore.Config.Player.MaxWeight then
            if (slot and self.PlayerData.items[slot]) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
                ...
                --Logs
                if amount >= 1000 then
                    TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'Added Item (Inventory)', 'green', '**'..self.PlayerData.charinfo.firstname .. ' '.. self.PlayerData.charinfo.lastname.. '** [ '..GetPlayerName(self.PlayerData.source) .. ' - Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..' ] got item: [slot:' ..slot..'], itemname: ' .. self.PlayerData.items[slot].name .. ', added amount: ' .. amount ..', new total amount: '.. self.PlayerData.items[slot].amount, true)
                else
                    TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'Added Item (Inventory)', 'green', '**'..self.PlayerData.charinfo.firstname .. ' '.. self.PlayerData.charinfo.lastname.. '** [ '..GetPlayerName(self.PlayerData.source) .. ' - Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..' ] got item: [slot:' ..slot..'], itemname: ' .. self.PlayerData.items[slot].name .. ', added amount: ' .. amount ..', new total amount: '.. self.PlayerData.items[slot].amount, false)
                end
            ...
            elseif (itemInfo['unique']) or (not slot or slot == nil) or (itemInfo['type'] == 'weapon') then
                for i = 1, QBConfig.Player.MaxInvSlots, 1 do
                    if self.PlayerData.items[i] == nil then
                        ...
                        self.Functions.UpdatePlayerData()
                        TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'Added Item (Inventory)', 'green', '**'..self.PlayerData.charinfo.firstname .. ' '.. self.PlayerData.charinfo.lastname.. '** [ '..GetPlayerName(self.PlayerData.source) .. ' - Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..' ] got item: [slot:' ..i..'], itemname: ' .. self.PlayerData.items[i].name .. ', added amount: ' .. amount ..', new total amount: '.. self.PlayerData.items[i].amount)
        ...
        if slot then
            if self.PlayerData.items[slot].amount > amount then
                ...
                if not self.Offline then
                    ...
                    TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'Removed Item (Inventory)', 'red', '**'..self.PlayerData.charinfo.firstname .. ' '.. self.PlayerData.charinfo.lastname.. '** [ '..GetPlayerName(self.PlayerData.source) .. ' - Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..' ] lost item: [slot:' ..slot..'], itemname: ' .. self.PlayerData.items[slot].name .. ', removed amount: ' .. amount ..', new total amount: '.. self.PlayerData.items[slot].amount, false)
                end
            ...
            elseif self.PlayerData.items[slot].amount == amount then
                ...
                if not self.Offline then
                    ...
                    TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'Removed Item (Inventory)', 'red', '**'..self.PlayerData.charinfo.firstname .. ' '.. self.PlayerData.charinfo.lastname.. '** [ '..GetPlayerName(self.PlayerData.source) .. ' - Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..' ] lost item: [slot:' .. slot..'], itemname: ' .. item .. ', removed amount: ' .. amount ..', item removed', false)
                end
                ...
                for _, _slot in pairs(slots) do
                    if self.PlayerData.items[_slot].amount > amountToRemove then
                        ...
                        if not self.Offline then
                            ...
                            TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**'..self.PlayerData.charinfo.firstname .. ' '.. self.PlayerData.charinfo.lastname.. '**  ['..GetPlayerName(self.PlayerData.source) .. ' - Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..' ] lost item: [slot:' .. _slot..'], itemname: ' .. self.PlayerData.items[_slot].name .. ', removed amount: ' .. amount ..', new total amount: '.. self.PlayerData.items[_slot].amount)
                        end
                    ...
                    elseif self.PlayerData.items[_slot].amount == amountToRemove then
                        ...
                        if not self.Offline then
                            ...
                            TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**'..self.PlayerData.charinfo.firstname .. ' '.. self.PlayerData.charinfo.lastname.. '** [ '..GetPlayerName(self.PlayerData.source) .. ' - Citizen ID: '..self.PlayerData.citizenid..' | ID: '..self.PlayerData.source..' ] lost item: [slot:' .. _slot..'], itemname: ' .. item .. ', removed amount: ' .. amount ..', item removed')
                        end
...
function QBCore.Player.DeleteCharacter(source, citizenid)
    ...
    if license == result then
        ...
        -- Delete from PlayerJobs & Police MDW Table
        MySQL.query('DELETE FROM player_jobs WHERE cid = ?', {citizenid})
        MySQL.query('DELETE FROM drx_mdw_officers WHERE citizenid = ?', {citizenid})
