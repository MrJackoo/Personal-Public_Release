...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	QBCore.Functions.Notify("Your property has been seized, you'll get everything back when your time is up", "error", 5000)
	DoScreenFadeOut(500)
	while not IsScreenFadedOut() do
		Wait(10)
	end
	SetEntityCoords(PlayerPedId(), 1767.41, 2486.56, 46.16, 1.0, 0, 0, 0, false)
	Wait(500)
	...
	Wait(2000)
	DoScreenFadeIn(1000)
	QBCore.Functions.Notify("Do some work to reduce your sentence - "..Config.Jobs[currentJob])
end)

-- Prison alarm reset
Citizen.CreateThread(function()
	exports['qb-target']:AddBoxZone("prisonalarmoff", vector3(1839.37, 2573.79, 46.01), 2.6, 1, {
		name = "prison_reset",
		debugPoly = false,
		minZ = 45.01,
		maxZ = 47.01,
		...
		},
		{
			options = {
				{
				type = "client",
				event = "prison:Client:ResetAlarm",
				icon = "fas fa-bell",
				label = "Reset Prison Alarm",
				job = "police",
				},
			},
			distance = 2.5
...
RegisterNetEvent('prison:Client:ResetAlarm', function()
...
RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		...
		if inJail then
			...
			RemoveBlip(ShopBlip)
			ShopBlip = nil
			QBCore.Functions.Notify("You have been released and your items have been returned", "success", 5000)
			DoScreenFadeOut(500)
			while not IsScreenFadedOut() do
				Wait(10)
			end
			...
			QBCore.Functions.Notify("You are not in jail, Do not try and Teleport", "error")
		end
	end
end)
	...
	while true do
		...
		if jailTime > 0 and inJail then
			...
			if jailTime > 0 and inJail then
				...
				if jailTime <= 0 then
					jailTime = 0
					QBCore.Functions.Notify("Check yourself out at the phone booth in visitation", "success", 10000)
				end
				TriggerServerEvent("prison:server:SetJailStatus", jailTime)
			end
		end
