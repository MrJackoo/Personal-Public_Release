...
local Gates = {
    [1] = {
        gatekey = "gabzprison-gabzprison_main_metal_1",
        coords = vector3(1845.99, 2604.7, 45.58),
        hit = false,
    },
    [2] = {
        gatekey = "gabzprison-gabzprison_main_metal_2",
        coords = vector3(1819.47, 2604.67, 45.56),
        hit = false,
    },
    [3] = {
        gatekey = "gabzprison-gabzprison_metal_ring_1",
...
RegisterNetEvent('qb-prison:Breakout', function()
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)    
    local dist = #(PlayerPos - vector3(1846.52, 2604.74, 45.58))
    if CurrentCops >= Config.MinimumPrisonPolice and not securityLockdown then 
        if dist <= 2.0 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    ...
                    TriggerServerEvent('cd_dispatch:AddNotification', {
                        job_table = {'police'}, 
                        coords = vector3(1693.33, 2569.51, 45.55),
                        title = 'Prison Break',
                        message = 'A prison break is in progress at HMP Bolingbroke',
                        flash = 0,
                        ...
                        blip = {
                            ...
                            colour = 4,
                            flashes = true,
                            text = 'Grade 1 - Prison Break',
                            time = (5*60*1000),
                            sound = 1,
                        }
                    })
                    QBCore.Functions.Progressbar("hack_gate", "Connecting to Gate Security System..", math.random(45000, 60000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        ...
                        StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                        TriggerEvent('qb-prison:gateHack1')
                    end, function() -- Cancel
                        StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
...
CreateThread(function()
    while true do
        ...
        if #(pos.xy - vec2(Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y)) > 200 and inJail then
            ...
            TriggerServerEvent('cd_dispatch:AddNotification', {
                job_table = {'police'}, 
                coords = vector3(1661.03, 2540.6, 45.56),
                title = 'Prison Break',
                message = 'A prison break is in progress at HMP Bolingbroke',
                flash = 0,
                unique_id = tostring(math.random(0000000,9999999)),
