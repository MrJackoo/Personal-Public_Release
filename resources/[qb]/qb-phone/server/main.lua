...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        SenderPly.Functions.AddMoney('bank', commission, 'Phone: Invoice Paid')
        invoiceMailData = {
            sender = 'Invoice Department',
            subject = 'Commission Received',
            message = string.format('You received a commission check of £%s when %s %s paid a invoice of £%s.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Invoice Department',
            subject = 'Invoice Paid',
            message = string.format('%s %s paid a invoice of £%s', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
...
--             sender = 'Invoice Department',
--             subject = 'Invoice Paid',
--             message = string.format('%s %s paid an invoice of $%s', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
--         }
--     end
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        if result[1] ~= nil then
            ...
            if Reciever ~= nil then
                Reciever.Functions.AddMoney('bank', amount, 'Phone: Transfer')
                if amount >= 25000 then
                    TriggerEvent('qb-log:server:CreateLog', 'money', 'Phone Transfer', 'blue', ''..Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname.. ' **(CitizenID: '..Player.PlayerData.citizenid..'** - ID: '..source..') Transferred **£'.. format_int(amount)..' ** to '..Reciever.PlayerData.charinfo.firstname .. ' ' .. Reciever.PlayerData.charinfo.lastname.. '( **Citizen ID: ' ..CID.. '**)', true)
                else
                    TriggerEvent('qb-log:server:CreateLog', 'money', 'Phone Transfer', 'blue', ''..Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname.. ' **(CitizenID: '..Player.PlayerData.citizenid..'** - ID: '..source..') Transferred **£'.. format_int(amount)..' ** to '..Reciever.PlayerData.charinfo.firstname .. ' ' .. Reciever.PlayerData.charinfo.lastname.. '( **Citizen ID: ' ..CID.. '**)', false)
                end
            else
                ...
                local perfaccount = MySQL.query.await('SELECT * FROM pefcl_accounts WHERE ownerIdentifier= ?', {result[1].citizenid})

                print("ADDING OFFLINE BALANCE OLD IS: ", perfaccount[1].balance, " New Balance is: ", perfaccount[1].balance + amount)
                -- Pefcl Update
                MySQL.update('UPDATE pefcl_accounts SET balance = ? WHERE ownerIdentifier = ?', {perfaccount[1].balance + amount, result[1].citizenid})

                if amount >= 25000 then
                    TriggerEvent('qb-log:server:CreateLog', 'money', 'Phone Transfer', 'blue', ''..Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname.. ' **(CitizenID: '..Player.PlayerData.citizenid..'** - ID: '..source..') Transferred **£'.. format_int(amount)..' ** to an offline person with the Citizen ID: ( **Citizen ID: ' ..CID.. '**)', true)
                else
                    TriggerEvent('qb-log:server:CreateLog', 'money', 'Phone Transfer', 'blue', ''..Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname.. ' **(CitizenID: '..Player.PlayerData.citizenid..'** - ID: '..source..') Transferred **£'.. format_int(amount)..' ** to an offline person with the Citizen ID: ( **Citizen ID: ' ..CID.. '**)', false)
                end
            end
            cb(true)
        else
...
QBCore.Functions.CreateCallback('qb-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
    -- removed police and nhs  xPlayer.PlayerData.job.name == 'police' or xPlayer.PlayerData.job.name == 'ambulance'
    for _, v in pairs(QBCore.Functions.GetPlayers()) do
        local xPlayer = QBCore.Functions.GetPlayer(v)
        local freelance = Player(v).state.freelance
        if xPlayer ~= nil then
            if (xPlayer.PlayerData.job.name ~= nil) and xPlayer.PlayerData.job.onduty then
                -- if freelance == 'taxi' then
                --     Lawyers[#Lawyers+1] = {
                ...
                -- else
                    Lawyers[#Lawyers+1] = {
                        name = xPlayer.PlayerData.charinfo.firstname .. ' ' .. xPlayer.PlayerData.charinfo.lastname,
                        phone = xPlayer.PlayerData.charinfo.phone,
                        typejob = xPlayer.PlayerData.job.name
                    }
                -- end
            end
        end
...
-- QBCore.Commands.Add('invoice', 'Invoice A Player', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
