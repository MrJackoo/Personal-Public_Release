...
RegisterNetEvent('qb-occasions:server:sellVehicleBack', function(vehData)
    ...
    for _, v in pairs(QBCore.Shared.Vehicles) do
        modelname = v['model']
        -- print(v['hash'], v['model'])
        -- print("Value hash is: ", GetHashKey(v['model']), " and the data hash is: ",vehData.model)
        if GetHashKey(v['model']) == vehData.model then
            price = tonumber(v["price"])
            break
        end
    end
    -- print("YO THE PRICE IS: £", price, " the plate is: ", plate, " the model is: ", vehData.model)
    local payout = math.floor(tonumber(price * 0.5)) -- This will give you half of the cars value
    -- print("YO THE PAYOUT IS: £", payout)

    Player.Functions.AddMoney('bank', payout, 'Vehicle Sales: Sold back '..modelname..' for 50% Price')
...
RegisterNetEvent('qb-occasions:server:buyVehicle', function(vehicleData)
    ...
    if result[1] and next(result[1]) then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            Player.Functions.RemoveMoney('bank', result[1].price, 'Purchased a second hand vehicle from lennys')
            MySQL.insert(
            ...
            if SellerData then
                SellerData.Functions.AddMoney('bank', NewPrice, 'Vehicle Sales: Sold a vehicle at lennys for Custom Price')
            else
