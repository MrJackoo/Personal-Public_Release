...
QBCore.Functions.CreateCallback("qb-garage:server:GetGarageVehicles", function(source, cb, garage, garageType, category)
    ...
    elseif garageType == "depot" then    --Depot give player cars that are not in garage only
        MySQL.query('SELECT * FROM player_vehicles WHERE citizenid = ? AND (state = ? OR garage = ? OR garage IS NULL)', {pData.PlayerData.citizenid, 3, garage}, function(result)
            local tosend = {}
            if result[1] then
...
if UseEnc0dedPersistenVehicles then
    QBCore.Functions.CreateCallback("qb-garage:server:checkIsSpawned", function (plate)
        local data = exports['persistent_cars']:GetVehicleData(plate, {'pos'})
...
AddEventHandler('onResourceStart', function(resource)
    if resource == GetCurrentResourceName() then
        Wait(100)
        if AutoRespawn then
            MySQL.update('UPDATE player_vehicles SET state = 3 WHERE state = 0', {})
        end
    end
end)
...
RegisterNetEvent('qb-garage:server:PayDepotPrice', function(data)
    ...
    if cashBalance >= depotPrice then
        Player.Functions.RemoveMoney("cash", depotPrice, 'Garages: Paid Depot')
    elseif bankBalance >= depotPrice then
        Player.Functions.RemoveMoney("bank", depotPrice, 'Garages: Paid Depot')
    else
        ...
        if result[1] then
            for k, v in pairs(result) do
                ...
                elseif v.state == 2 then
                    v.state = Lang:t("status.policeimpound")
