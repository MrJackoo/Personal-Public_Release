		...
		if name and id then
			...
			elseif name == "otherplayer" then
				...
				if OtherPlayer then
					...
					elseif OtherPlayer.PlayerData.job.name ~= "police" then
						secondInv.slots = QBCore.Config.Player.MaxInvSlots
					else
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				...
				if (string.match(fromItemData.name, "backpack") and string.match(toInventory, "backpack")) then
					return
				end
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "Item Purchase") then
                if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
                    itemData.info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
                end
				if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "fishicebox" then
                    itemData.info.boxid = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
				end	
				Player.Functions.AddItem(itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('qb-shops:client:UpdateShop', src, QBCore.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
				TriggerClientEvent('QBCore:Notify', src, itemInfo["label"] .. " bought!", "success")
				--TriggerEvent("qb-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for £"..price)
				TriggerEvent("qb-log:server:CreateLog", "shops", "Shop Purchase", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ ".. GetPlayerName(src) .. " - Citizen ID: "..Player.PlayerData.citizenid.." | ID:"..src.." ] bought **" .. fromAmount .. " x ".. itemInfo["label"] .. " for £"..price.."**")
			elseif bankBalance >= price then
				Player.Functions.RemoveMoney("bank", price, "Item Purchase")
                if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
                    itemData.info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
                end
				Player.Functions.AddItem(itemData.name, fromAmount, toSlot, itemData.info)
		...
		if amount > 0 then
			if itemData then
				...
				elseif itemData["name"] == "harness" then
					info.uses = 50
				elseif itemData["name"] == "laptop1" then
					info.uses = 3
				elseif itemData["name"] == "laptop2" then
					info.uses = 3
				elseif itemData["name"] == "laptop3" then
					info.uses = 3
				elseif itemData["name"] == "laptop4" then
					info.uses = 3
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('QBCore:Notify', src, "You Have Given " ..GetPlayerName(tonumber(args[1])).." "..amount.." "..itemData["name"].. "", "success")
					TriggerEvent("qb-log:server:CreateLog", "staff", "Staff Command", "blue", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.. "** [ "..GetPlayerName(src) .. " - Citizen ID: "..Player.PlayerData.citizenid.." | ID: "..Player.PlayerData.source.." ] Has Given **" ..GetPlayerName(tonumber(args[1])).."** **"..amount.."** x **"..itemData["name"].."**")
				else
