...
function EmoteCancel()
    ...
    for k, v in pairs(GetGamePool('CObject')) do
        ...
        local holsterdata = exports['holsters']:getholsters()
        if holsterdata then
            for key, attached_object in pairs(holsterdata) do
                DeleteObject(attached_object.Object)
                for k,v in pairs(holsterdata[key].Components) do
...
RegisterNetEvent('customanimation:dialog', function()
    local animdialog = exports['qb-input']:ShowInput({
      header = "Player Animation from Dictionary",
      submitText = "Play Anim",
      inputs = {
          {
              text = "Animation Dictionary",
              name = "dict",
              type = "text",
              isRequired = true
          },
    ...
    if not animdialog then return end
    TriggerEvent('customanimation:play', animdialog.dict, animdialog.animname)
  end)
  
  RegisterNetEvent('customanimation:play', function(dict, anim)
    ...
    TaskPlayAnim(PlayerPedId(), dict, anim, 1.0, 1.0, -1, 1, 1, false, false, false)
  end)
  
  RegisterNetEvent('customanimation:stop', function(args)
    local ped = PlayerPedId()
    ClearPedTasks(ped)
  end)